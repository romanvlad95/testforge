...
def test_validate_csv_full_coverage(tmp_path):
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        writer.writerow(
            ["abc", "bademail", "pending", "Some note"]
        )  # int fail, regex fail, enum fail
        writer.writerow(
            ["50", "user@example.com", "active", "   "]
        )  # int below min, whitespace string
...
def test_regex_validation_failure(tmp_path):
    ...
    schema = [{"name": "username", "type": "str", "constraints": {"regex": r"^\w+$"}}]
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
